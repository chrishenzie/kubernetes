...
func (f *cgroupOptionsFeature) Discover(cfg *types.NodeConfiguration) bool {
	...
	// Writable cgroups require cgroup v2 with nsdelegate. CgroupsPerQOS provides
	// the pod cgroup on which the kubelet sets descendant and depth limits.
	// RuntimeFeatures.CgroupMountMode reports runtime API support. StaticConfig
	// supplies the node requirements.
...
func (f *cgroupOptionsFeature) InferForScheduling(podInfo *types.PodInfo) bool {
	for i := range podInfo.Spec.Containers {
		if requestsWritableCgroups(podInfo.Spec.Containers[i].SecurityContext) {
			return true
		}
	}
	for i := range podInfo.Spec.InitContainers {
		if requestsWritableCgroups(podInfo.Spec.InitContainers[i].SecurityContext) {
			return true
		}
	}
	return false
...
func requestsWritableCgroups(sc *v1.SecurityContext) bool {
