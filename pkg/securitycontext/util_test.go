...
func TestDetermineEffectiveSecurityContextCgroupOptions(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// The merge path only copies the container fields it knows about, so a pod
			// that sets any pod-level securityContext would otherwise lose cgroupOptions.
			name:   "pod security context set",
			podSc:  &v1.PodSecurityContext{RunAsNonRoot: ptr.To(true)},
			expect: &v1.CgroupOptions{MountMode: &writable},
		},
	}
